...
export default function WeatherScreen() {
  ...
  const loadForecast = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        console.log("WeatherScreen error:", e.message);
        setFetchError(true);
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  useEffect(() => {
    loadForecast();
    Analytics.logScreenView("WeatherScreen");
    Analytics.logWeatherCheck(location?.lat, location?.lon);
  }, [loadForecast]);
          ...
          return (
            <View key={date} style={[styles.row, isToday && styles.rowToday]}>
              ...
              <View style={styles.infoCol}>
                <Text
                  style={[styles.dateLabel, isToday && styles.dateLabelToday]}
                >
                  {isToday ? t("today") : formatDate(date)}
                </Text>
                <Text style={styles.condLabel}>
                  {t(getWmoKey(forecast.weathercode[i]))}
                </Text>
                <View style={styles.rainRow}>
                  <Text style={styles.rainIcon}>🌧️</Text>
                  <Text style={styles.rainText}>
                    {forecast.precipitation_sum[i]?.toFixed(1)} mm
                  </Text>
                </View>
              </View>
              <View style={styles.tempCol}>
                <Text style={styles.maxTemp}>
                  ↑ {forecast.temperature_2m_max[i]?.toFixed(0)}°
                </Text>
                <Text style={styles.minTemp}>
                  ↓ {forecast.temperature_2m_min[i]?.toFixed(0)}°
                </Text>
              </View>
            </View>
          );
