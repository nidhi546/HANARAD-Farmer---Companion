...
const FormInput = memo(
  React.forwardRef(function FormInput(
    {
      label,
      value,
      onChangeText,
      ...
      <>
        <View style={fi.row}>
          <Text style={[fi.label, { color: theme.subtext }]}>
            {label}
            {required ? <Text style={fi.required}> *</Text> : null}
          </Text>
          <TextInput
            ref={ref}
            style={[
        ...
        {!isLast && (
          <View style={[fi.divider, { backgroundColor: theme.border }]} />
        )}
      </>
    );
  }),
...
const DOBField = memo(function DOBField({
  label,
  value,
  onChange,
  error,
      ...
      <Modal visible={showPicker} transparent animationType="slide">
        ...
        <View style={dobs.sheet}>
          <View style={dobs.header}>
            ...
            <TouchableOpacity
              onPress={() => {
                if (iosTempDate) onChange(formatDate(iosTempDate));
                setShowPicker(false);
              }}
              hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
            >
...
export default function EditProfileScreen({ navigation }) {
  ...
  const onChangeUsername = useCallback((v) => {
    setUsername(v);
  ...
  const onChangePhone = useCallback((v) => {
    setPhone(v);
    setErrors((e) => ({ ...e, phone: null }));
  }, []);
  const onChangeDob = useCallback((v) => {
    setDob(v);
  ...
  const onChangeCity = useCallback((v) => {
    setCity(v);
    setErrors((e) => ({ ...e, city: null }));
  }, []);
  const onChangeState_ = useCallback((v) => {
    setState_(v);
    setErrors((e) => ({ ...e, state: null }));
  }, []);
  const onChangeCountry = useCallback((v) => {
    setCountry(v);
      ...
      [
        {
          text: t("fromCamera"),
          onPress: async () => {
            const u = await pickFromCamera(t);
            if (u) setPhoto(u);
          },
        },
        {
          text: t("fromGallery"),
          onPress: async () => {
            const u = await pickFromGallery(t);
            if (u) setPhoto(u);
          },
        },
        photo
    ...
    <SafeAreaView style={styles.root} edges={["top"]}>
          ...
          <View style={styles.completionBox}>
            ...
            <View style={styles.progressTrack}>
              <View
                style={[
                  styles.progressFill,
                  { width: `${completion}%`, backgroundColor: completionColor },
                ]}
              />
            </View>
          </View>
            ...
            <TouchableOpacity onPress={handlePhoto} activeOpacity={0.85}>
              <View style={styles.photoWrap}>
                {photo ? (
                  ...
                  <View style={styles.photoInit}>
                    <Text style={styles.photoInitTxt}>{getInitials(name)}</Text>
                  </View>
                )}
                ...
                {[
                  ...
                  >
                    <Text
                      style={[
                        styles.langBtnTxt,
                        language === l.code && styles.langBtnTxtActive,
                      ]}
                    >
                      {l.label}
                    </Text>
      ...
      >
        {saving ? (
          ...
          <>
            ...
            <Text style={styles.saveBtnTxt}>{t("saveChanges")}</Text>
          </>
        )}
      </TouchableOpacity>
      ...
      {toast && (
        <View
          style={[
            styles.toast,
            { backgroundColor: toast.ok ? "#DCFCE7" : "#FEF2F2" },
          ]}
        >
